// Export pages
export '/login_perfil/login/login_widget.dart' show LoginWidget;
export '/main/paginade_ingreso/paginade_ingreso_widget.dart'
    show PaginadeIngresoWidget;
export '/login_perfil/recuperar_contra/recuperar_contra_widget.dart'
    show RecuperarContraWidget;
export '/ajustes/ajustes/ajustes_widget.dart' show AjustesWidget;
export '/login_perfil/cambiar_contrasena/cambiar_contrasena_widget.dart'
    show CambiarContrasenaWidget;
export '/login_perfil/perfil/perfil_widget.dart' show PerfilWidget;
export '/main/menu/menu_widget.dart' show MenuWidget;
export '/calendario/ver_reservas/ver_reservas_widget.dart'
    show VerReservasWidget;
export '/calendario/agregar_actividad/agregar_actividad_widget.dart'
    show AgregarActividadWidget;
export '/instructores/inventario_instructores/inventario_instructores_widget.dart'
    show InventarioInstructoresWidget;
export '/administradores/inventario_administradores/inventario_administradores_widget.dart'
    show InventarioAdministradoresWidget;
export '/proveedores/inventario_proveedores/inventario_proveedores_widget.dart'
    show InventarioProveedoresWidget;
export '/equipo/inventario_equipo/inventario_equipo_widget.dart'
    show InventarioEquipoWidget;
export '/clases/inventario_clases/inventario_clases_widget.dart'
    show InventarioClasesWidget;
export '/principal/clientes/inventario_clientes/inventario_clientes_widget.dart'
    show InventarioClientesWidget;
export '/principal/productos/inventario_productos/inventario_productos_widget.dart'
    show InventarioProductosWidget;
